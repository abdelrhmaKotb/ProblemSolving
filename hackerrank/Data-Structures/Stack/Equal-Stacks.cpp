...
int equalStacks(vector<int> h1, vector<int> h2, vector<int> h3) {
    ...
    while (((sum1 == sum2) && sum2 == sum3) == false) {
        min = std::min(sum1,std::min(sum2,sum3));
       
        if (sum1 > min && i1 != h1.end()) {
            sum1 -= (*i1);
            i1++;  
        }
        
        if (sum2 > min && i2 != h2.end()) {
            sum2 -= (*i2);
            i2++;  
        }
        if (sum3 > min && i3 != h3.end()) {
            sum3 -= (*i3);
            i3++;  
        }
